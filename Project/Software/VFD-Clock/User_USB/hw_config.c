...
/*******************************************************************************
* Function Name  : USB_PreInit
* Description    : Configures USB clocks, connect pin, and interrupt.
*									 本函数将初始化中用到的本文件的其他函数封装了起来
* Input          : None.
* Return         : None.
*******************************************************************************/
void USB_PreInit(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = USB_DISCONNECT_PIN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = USB_GPIO_MODE;
  GPIO_Init(USB_DISCONNECT, &GPIO_InitStructure);
